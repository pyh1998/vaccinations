...
def global_population_vaccinated(table):
    ...
    for i in range(len(countries_people_vaccinated)):
        if (i == len(countries_people_vaccinated) - 1 or countries_people_vaccinated[i][0] !=
                countries_people_vaccinated[i + 1][0]):
...
def global_fully_population_vaccinated(table):
    ...
    for i in range(len(countries_people_fully_vaccinated)):
        if (i == len(countries_people_fully_vaccinated) - 1 or countries_people_fully_vaccinated[i][0] !=
                countries_people_fully_vaccinated[i + 1][0]):
...
def vaccinations_by_country(table):
    countries_vaccinations_rate = [[row[0], row[4], float(row[10]), round(float(row[10]) - float(row[11]), 2)] for row
                                   in table if "OWID_" not in row[1] and row[4] != ""
                                   and row[10] != "" and row[11] != "" and float(row[10]) != 0.0 and int(
            row[4]) / float(row[10]) > 10000]
    result = []
    for i in range(len(countries_vaccinations_rate)):
        if (i == len(countries_vaccinations_rate) - 1 or countries_vaccinations_rate[i][0] !=
                countries_vaccinations_rate[i + 1][0]):
...
def earliest_countries(table):
    ...
    for country_date in result[0:10]:
        each_country_daily_vaccinations = [[row[0], int(row[1]), row[2]] for row in countries_date if
                                           country_date[0] == row[0] and row[1] != ""]
        max_daily_vaccinations = max(each_country_daily_vaccinations)
        print(country_date[0] + ": first vaccinated on " + country_date[2] + " , " + str(
            max_daily_vaccinations[1]) + " people vaccinated on " + max_daily_vaccinations[2])
...
def fully_vaccinated_rate(table):
    countries_fully_vaccinated = [[row[0], float(row[11])] for row in table if
                                  "OWID_" not in row[1] and row[11] != "" and float(row[11]) > 50]
    result = []
    # result.append(countries_date[0])
    for i in range(len(countries_fully_vaccinated)):
        if (i == len(countries_fully_vaccinated) - 1 or countries_fully_vaccinated[i][0] !=
                countries_fully_vaccinated[i + 1][0]):
